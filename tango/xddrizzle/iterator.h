...
#ifndef __XDDRIZZLE_ITERATOR_H
#define __XDDRIZZLE_ITERATOR_H
...
public:
    ...
    void init(drizzle_result_st* result)
    {
        m_result = result;
    }
    ...
    drizzle_return_t next()
    {
        if (m_eof)
            return DRIZZLE_RETURN_OK;
        if (m_row)
            drizzle_row_free(m_result, m_row);
        drizzle_return_t ret;
        m_row = drizzle_row_buffer(m_result, &ret);
        if (!m_row)
            m_eof = true;
        return ret;
    }
    ...
    size_t* getFieldLengths() { return drizzle_row_field_sizes(m_result); }
...
class DrizzleIterator : public CommonBaseIterator,
                      public tango::ICacheRowUpdate
{
friend class DrizzleDatabase;
friend class DrizzleSet;

    XCM_CLASS_NAME("xddrizzle.Iterator")
    XCM_BEGIN_INTERFACE_MAP(DrizzleIterator)
...
private:
    ...
    UnbufferedReader m_reader;
    
    std::vector<DrizzleDataAccessInfo*> m_fields;
    std::vector<DrizzleDataAccessInfo*> m_exprs;

    LocalRowCache m_cache;
    LocalRow m_cache_row;
    char** m_cache_row_ptrs;      // holds the results of a row
