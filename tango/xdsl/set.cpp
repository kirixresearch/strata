...
{
    return m_dbint->getCatalogPath(m_ordinal);
}
...
tango::IStructurePtr SlSet::getStructure()
{
    ...
    m_structure = m_dbint->getStructureFromOrdinal(m_ordinal);
...
int SlSet::insert(tango::IIteratorPtr source_iter,
                      ...
                      tango::IJob* job)
{
    return xdcmnInsert(source_iter, this, constraint, max_rows, job);
}
...
tango::IIteratorPtr SlSet::createBookmark(tango::rowid_t rowid)
{
    return xcm::null;
}

bool SlSet::rowInSet(tango::rowid_t rowid)
{
    return false;
}
...
bool SlRowInserter::startInsert(const std::wstring& col_list)
{
    ...
    for (i = 0; i < col_count; ++i)
    {
        ...
        data.idx = i+1;
        data.name = cols[i];
        data.str_data = new char[data.length * 5];
