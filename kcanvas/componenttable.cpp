...
bool CompTableModel::load(IStoreValuePtr store)
{
    ...
    // load the row and column counts
    IStoreValuePtr node_row_count;
    node_row_count = store->getChild(wxT("row.count"), false);
    if (node_row_count.isNull())
        return false;
        
    setRowCount(node_row_count->getInteger());
    
    IStoreValuePtr node_column_count;
    node_column_count = store->getChild(wxT("column.count"), false);
    if (node_column_count.isNull())
        return false;

    setColumnCount(node_column_count->getInteger());

    // load the cell range count
    IStoreValuePtr node_1;
    node_1 = store->getChild(wxT("cell.range.count"), false);
    
    if (node_1.isNull())
        return false;

    int cell_range_count = node_1->getInteger();

    // iterate through the groups of cell properties
    int range_idx;
    for (range_idx = 0; range_idx < cell_range_count; ++range_idx)
    {
        // load the node to stores this group of cell properties
        wxString node_2_name = wxString::Format(wxT("cell.range%d"), range_idx);

        IStoreValuePtr node_2;
        node_2 = store->getChild(node_2_name, false);
        
        // if for some reason, we can't load it, move on
        if (node_2.isNull())
            continue;

        // load the node that stores the range of cell properties
        IStoreValuePtr node_2_1;
        node_2_1 = node_2->getChild(wxT("cell.range.value"), false);

        // if we can't find the cell range values, move on
        if (node_2_1.isNull())
            continue;

        // get the cell ranges values from the property string
        wxString token = node_2_1->getString();
        ...
        while(1)
        {
            wxString range_part = token.BeforeFirst(wxT(';'));
        ...
        // get the properties node for this range
        IStoreValuePtr node_2_2;
        node_2_2 = node_2->getChild(wxT("cell.range.properties"), false);
        
        // if we don't have properties for this range, move on
        if (node_2_2.isNull())
            continue;
        
        // load the properties
        Properties properties;
        if (!loadProperties(node_2_2, properties))
            continue;

        // if we don't have any properties to set, move on
        if (properties.count() == 0)
            continue;

        // add the properties to the list of cell properties
        CellProperties cell_properties;
        cell_properties.setRange(row1, col1, row2, col2);
        cell_properties.addProperties(properties);
        m_cell_properties.push_back(cell_properties);
    }

    // load the cell merge count
    IStoreValuePtr node_3;
    node_3 = store->getChild(wxT("cell.merge.count"), false);
    
    if (node_3.isNull())
        return false;

    int cell_merge_count = node_3->getInteger();

    // iterate through the groups of merged cells
    int merge_idx;
    for (merge_idx = 0; merge_idx < cell_merge_count; ++merge_idx)
    {
        // load the node to stores this group of merged cells
        wxString node_4_name = wxString::Format(wxT("cell.merge%d"), merge_idx);

        IStoreValuePtr node_4;
        node_4 = store->getChild(node_4_name, false);
        
        // if for some reason, we can't load it, move on
        if (node_4.isNull())
            continue;

        // load the node that stores the range of the merged cells
        IStoreValuePtr node_4_1;
        node_4_1 = node_4->getChild(wxT("cell.merge.value"), false);
        ...
        // get the cell ranges values from the property string
        wxString token = node_4_1->getString();
        ...
        CellProperties merged_cells;
        merged_cells.setRange(row1, col1, row2, col2);;
        m_cell_merges.push_back(merged_cells);
    }
