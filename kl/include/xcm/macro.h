...
#define XCM_BEGIN_TYPE_INFO(interf)    \
    public:    \
    static void* xcm_interface_runtime(    \
        interf* pI, const char* call_method, xcm::value* params, xcm::value* retval){ \
            static xcm::interface_info ti; \
            if (!pI) { \
                ti.clear(); \
                ti.set_name(xcm_get_interface_name()); \
            }

#define XCM_METHOD_R0(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        *retval = pI->method_name(); \
        return NULL; \
    }
#define XCM_METHOD_R1(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        *retval = pI->method_name(*params); \
        return NULL; \
    }
#define XCM_METHOD_R2(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        *retval = pI->method_name(*params, *(params+1)); \
        return NULL; \
    }
#define XCM_METHOD_R3(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        *retval = pI->method_name(*params, *(params+1), *(params+2)); \
        return NULL; \
    }
#define XCM_METHOD_V0(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        pI->method_name(); \
...
#define XCM_METHOD_V1(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        pI->method_name(*params); \
        return NULL; \
    }
#define XCM_METHOD_V2(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        pI->method_name(*params, *(params+1)); \
        return NULL; \
    }
#define XCM_METHOD_V3(method_name, declaration)    \
    if (!pI) \
        ti.add_method_info().parse_decl(declaration); \
    if (pI && !strcmp(call_method, #method_name)) { \
        pI->method_name(*params, *(params+1), *(params+2)); \
        return NULL; \
    }


#define XCM_END_TYPE_INFO() \
        if (!pI) \
            return (void*)&ti; \
        return NULL; \
    } \
    static xcm::interface_info* xcm_get_type_info() { \
        return (xcm::interface_info*)xcm_interface_runtime(NULL,NULL,NULL,NULL); \
    }
