...
DuckdbStream::DuckdbStream(DuckdbDatabase* database)
{
    m_mime_type = L"application/octet-stream";
    m_offset = 0;
    m_block_size = DuckdbStream::DEFAULT_BLOCK_SIZE;

    m_database = database;
    m_database->ref();
}

DuckdbStream::~DuckdbStream()
{
    m_database->unref();
}

bool DuckdbStream::init(const std::wstring& stream_object_name, const std::wstring& mime_type, int block_size)
{
...
bool DuckdbStream::readBlock(long long block, void* buf, unsigned long* read_count)
{
...
bool DuckdbStream::writeBlock(long long block, const void* buf, unsigned long size)
{
...
bool DuckdbStream::read(void* buf,
...
bool DuckdbStream::write(const void* buf,
...
bool DuckdbStream::seek(long long seek_pos, int whence)
{
    switch (whence)
    {
        case xd::seekSet:
...
long long DuckdbStream::getSize()
{
    // first, find out the largest numbered block already in the table
...
std::wstring DuckdbStream::getMimeType()
{
    return m_mime_type;
}
