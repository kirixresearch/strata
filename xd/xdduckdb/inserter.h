...
#ifndef H_XDDUCKDB_INSERTER_H
#define H_XDDUCKDB_INSERTER_H

#include "duckdbinc.h"


class DuckdbRowInserterData
{
public:
...
class DuckdbRowInserter : public xd::IRowInserter
{
    XCM_CLASS_NAME("xdsl.RowInserter")
    XCM_BEGIN_INTERFACE_MAP(DuckdbRowInserter)
        XCM_INTERFACE_ENTRY(xd::IRowInserter)
    XCM_END_INTERFACE_MAP()

public:

    DuckdbRowInserter(DuckdbDatabase* database, const std::wstring& table);
    virtual ~DuckdbRowInserter();
...
private:
    ...
    std::vector<DuckdbRowInserterData> m_fields;
};
