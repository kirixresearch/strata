...
bool XdnativeDatabase::loadDefinition(const std::wstring& path, xd::FormatDefinition* format_info)
{
    std::wstring cstr, rpath;
    if (detectMountPoint(path, &cstr, &rpath))
    {
        ...
        return db->loadDefinition(rpath, format_info);
    }
...
xd::IRelationEnumPtr XdnativeDatabase::getRelationEnum(const std::wstring& path)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        if (filter_active)
        {
            ...
            if (filter_set_path.length() > 0 && left_set_path.length() > 0 && !isSamePath(filter_set_path, left_set_path))
                continue;
        }
...
xd::ColumnInfo XdnativeDatabase::validateExpression(const std::wstring& expression, const xd::Structure& structure, const std::wstring& path_context)
{
