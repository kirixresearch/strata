...
set VC_OUTPUT_PATH=c:\build\x86\src\appstrata\releaseu
...
set BUILDUTIL=cscript /nologo %SOURCE_PATH%\appmain\buildutil.vbs c:\build\common
...
%BUILDUTIL% set_version %WEBRES_DIR%\about\about.html %BUILDSRC%\webres\about\about.html
%BUILDUTIL% set_version %WEBRES_DIR%\about\license.html %BUILDSRC%\webres\about\license.html
%BUILDUTIL% set_version %WEBRES_DIR%\about\oss.html %BUILDSRC%\webres\about\oss.html
...
REM %WIX_PATH%\candle %SETUP_PATH%\xr.wxs -o %SETUP_PATH%\xr.wixobj
%WIX_PATH%\candle %SETUP_PATH%\%WXS_NAME%_tmp.wxs -o %SETUP_PATH%\%WXS_NAME%.wixobj
