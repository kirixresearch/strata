...
// -- PkgStreamEnum class implementation --
...
// -- PkgStreamReader class implementation --
...
// -- PkgStreamWriter class implementation --
...
PkgStreamWriter::~PkgStreamWriter()
{
    // -- make sure call was completed with finishWrite();
    assert(!m_writing);
...
bool PkgFile::createDirEntry(const PkgDirEntry& entry)
{
    ...
    int2buf(dir_entry+4, lo);
    int2buf(dir_entry+8, hi);

    // -- set file offset of the stream --
    hi = entry.first_block_offset >> 32;
    lo = entry.first_block_offset & 0xffffffff;

    int2buf(dir_entry+12, lo);
    int2buf(dir_entry+16, hi);

    // -- set stream name --
    if (m_version == 1)
    {
        strcpy((char*)dir_entry+32, tostr(entry.stream_name).c_str());
    }
     else
    {
