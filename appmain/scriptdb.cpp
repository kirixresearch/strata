...
int DbDatabaseType::toConnectionDatabaseType(int binding_database_type)
{
    switch (binding_database_type)
    {
        // TODO: FIXME: there are missing types from this list, Xbase, etc.
        case DbDatabaseType::Xdnative:     return dbtypeXdnative;          break;
        case DbDatabaseType::Package:      return dbtypePackage;           break;
        case DbDatabaseType::Oracle:       return dbtypeOracle;            break;
        case DbDatabaseType::Postgres:     return dbtypePostgres;          break;
        case DbDatabaseType::SqlServer:    return dbtypeSqlServer;         break;
        case DbDatabaseType::MySQL:        return dbtypeMySql;             break;
        case DbDatabaseType::Odbc:         return dbtypeOdbc;              break;
        case DbDatabaseType::Access:       return dbtypeAccess;            break;
        case DbDatabaseType::Excel:        return dbtypeExcel;             break;
        case DbDatabaseType::Filesystem:   return dbtypeFilesystem;        break;
        default:                           return dbtypeUndefined;         break;
    }
}
...
{
         if (str == wxT("mssql"))             return DbDatabaseType::SqlServer;
    else if (str == wxT("mysql"))             return DbDatabaseType::MySQL;
    else if (str == wxT("oracle"))            return DbDatabaseType::Oracle;
    else if (str == wxT("postgres"))          return DbDatabaseType::Postgres;
    else if (str == wxT("odbc"))              return DbDatabaseType::Odbc;
    else if (str == wxT("db2"))               return DbDatabaseType::Db2;
    else if (str == wxT("package"))           return DbDatabaseType::Package;
    else if (str == wxT("msaccess"))          return DbDatabaseType::Access;
    else if (str == wxT("msexcel"))           return DbDatabaseType::Excel;
    else if (str == wxT("xbase"))             return DbDatabaseType::Filesystem;
    else if (str == wxT("delimited_text"))    return DbDatabaseType::Filesystem;
    else if (str == wxT("fixed_length_text")) return DbDatabaseType::Filesystem;
    else if (str == wxT("sqlite"))            return DbDatabaseType::Sqlite;
                        ...
                        const std::wstring& _columns)
{
     ...
     else
    {
        ...
        while (t.HasMoreTokens())
        {
            ...
            tango::IColumnInfoPtr sp_col = structure->getColumnInfo(towstr(token));
            if (!sp_col)
            {
...
void DbConnection::constructor(kscript::ExprEnv* env, kscript::Value* retval)
{
    if (env->getParamCount() >= 1)
    {
        ...
        if (cstr.empty())
        {
            // passing an empty string connects to the hostapp's current db
            m_db = g_app->getDatabase();
        }
         else
        {
...
void DbConnection::connect(kscript::ExprEnv* env, kscript::Value* retval)
{
    ...
    if (env->getParamCount() < 1)
    {
        // passing no param connects to the hostapp's current db
        m_db = g_app->getDatabase();
    }
     else
    {
