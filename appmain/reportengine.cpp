...
static wxString getOrderByField(const wxString& field)
{
    ...
    return xd::dequoteIdentifier(g_app->getDatabase(), query_field);
}
...
static wxString getSortExprFromGroups(const std::vector<ReportSection>& sections)
{
    ...
    for (it = sections.begin(); it != it_end; ++it)
    {
        ...
        wxString group_field = xd::quoteIdentifier(g_app->getDatabase(), towstr(it->m_group_field));
...
void ReportLayoutEngine::populateDataModel()
{
    ...
    else
    {
        ...
        wxString quoted_data_source = xd::quoteIdentifier(g_app->getDatabase(), towstr(data_source));        
